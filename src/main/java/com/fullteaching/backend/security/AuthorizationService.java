...
public class AuthorizationService {
	...
	public ResponseEntity<Object> checkAuthorizationUsers(Object o, Collection<User> users){
		...
		if(!users.contains(this.user.getLoggedUser())){
			//The user is not authorized to edit if it is not an attender of the com.fullteaching.backend.Course
			log.error("com.fullteaching.backend.User not authorized. Must be a participant of the course");
			return new ResponseEntity<>(HttpStatus.UNAUTHORIZED); 
		}
		return null;
	}
