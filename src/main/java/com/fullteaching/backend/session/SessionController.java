	...
	public ResponseEntity<Object> deleteSession(@PathVariable(value="id") String id) {
		...
		if (teacherAuthorized != null) { // If the user is not the teacher of the course
			...
			if (course != null){
				...
				//The com.fullteaching.backend.Course that owns the deleted session does not exist
				//This code is presumed to be unreachable, because of the Cascade.ALL relationship from com.fullteaching.backend.Course to Session
