...
export class CourseDetailsComponent implements OnInit {
  ...
  onCourseDetailsSubmit() {
    ...
    //If modal is opened in "New com.fullteaching.backend.Comment" mode (replaying or not replaying)
  ...
  deleteAttender(attender: User, j: number) {
    ...
    for (let i = 0; i < this.course.attenders.length; i++){
      if (this.course.attenders[i].id !== attender.id) {
        c.attenders.push(new User(this.course.attenders[i])); //Inserting a new com.fullteaching.backend.User object equal to the attender but "courses" array empty
      }
    }
